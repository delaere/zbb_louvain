...
.L printOut.C
printOut("Directory is ",DIR.Data())

// Output name
TString NN("ZH"+mass+"_"+channel+struct+"_"+iter+"_"+s_cuts);
printOut("OUTPUT NAME :", NN.Data())
...
int iterations=$4;
printOut("NNStruct ",NNStruct.Data())
printOut("iterations ","", iterations)

//choose the cuts
TString cuts("")
if(s_cuts.Contains("MM")) cuts+=" && rc_eventSelection_18_idx==1";

if(s_cuts.Contains("Mbb45-115")) cuts+=" && (eventSelectiondijetM>45 && eventSelectiondijetM<115)";
if(s_cuts.Contains("Mbb15-115")) cuts+=" && (eventSelectiondijetM>15 && eventSelectiondijetM<115)";
...
cat ${dir}/logroot_${choice}_${mass}_${channel}${struct}_${iter}_${s_cuts}.txt
...
.L printOut.C
printOut("numberOfPoints=","",numberOfPoints)
...
.q
EOF
   limfile=CSV_nosys_test.txt
   if [[ "${choice}" == *"ZZ_vs_"* ]]
   then
       limfile=CSV_nosys_testZZ.txt
   fi
   if [[ "${choice}" == *"BDTZZ"* ]]
   then
       limfile=CSV_nosys_testZZ.txt
   ...
   cat ${limfile} | sed -e s%ROOTFILE%${dirNN}/${dir}/NN_${choice}_ZH${mass}_${channel}${struct}_${iter}_${s_cuts}.root%g > ${dir}/CSV_nosys_${choice}_ZH${mass}_${channel}${struct}_${iter}_${s_cuts}.txt
   cat ${limfile} | sed -e s%ROOTFILE%${dirNN}/${dir}/NN_${choice}_ZH${mass}_${channel}${struct}_${iter}_${s_cuts}.root%g | sed -e s%Channel/%Channel/prod%g > ${dir}/CSV_nosys_prod_${choice}_ZH${mass}_${channel}${struct}_${iter}_${s_cuts}.txt
   ...
   cat ${dirNN}/${dir}/loglimitProd_${choice}_${mass}_${channel}${struct}_${iter}_${s_cuts}.txt
   cd ${dirNN}
   eval `scramv1 runtime -sh`
fi