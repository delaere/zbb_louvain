...
void Likelihood_minmisation(const char *inputFile)
{
    ...
    //---------------------------------------------------------
	TMinuit *gMinuit = new TMinuit(11);        // initialize TMinuit with 15 params
...
// Initialisation of 15 parameters.
   //------------------------------------------------Start----Step --- range-------
   gMinuit->mnparm(0, "mean gauss 1 constant     ", -1.9 , 0.01  , -8.0,5.0,ierflg);
   gMinuit->mnparm(1, "mean gauss 1 E deps       ", 0.00092,0.002,-0.2,0.2,ierflg);
   gMinuit->mnparm(2, "mean gauss 1 sqrt(E) deps ", 0.0,0.0,0.0,0.0,ierflg);
   
   gMinuit->mnparm(3, "width gauss 1 constant    ", 2.2056 , 0.00001 , 0.0,8.0,ierflg);
   gMinuit->mnparm(4, "width gauss 1 E deps      ", 0.0069 , 0.0001 , 0.0,0.5,ierflg);
   gMinuit->mnparm(5, "width gauss 1 sqrt(E) deps", 0.085 ,0.002,0.0,2.0,ierflg);

   gMinuit->mnparm(6, "ratio constant            ", 0.0 , 0.0 , 0.0,0.0,ierflg);
   gMinuit->mnparm(7, "ratio E deps              ", 0.001, 0.00002 , -0.01,0.01,ierflg);
   gMinuit->mnparm(8, "ratio sqrt(E) deps        ", 0.014 ,0.00001,0.0,0.1,ierflg);
   
   gMinuit->mnparm(9, "mean gauss 2 constant     ", 5.0, 1.0 , 1.0,80.0,ierflg); 
   gMinuit->mnparm(10,"mean gauss 2 E deps       ", 0.013 , 0.0 , 0.0,0.0,ierflg);
   gMinuit->mnparm(11,"mean gauss 2 sqrt(E) deps ", 0.1, 0.02,-2.0,8.0,ierflg);
   
   gMinuit->mnparm(12,"width gauss 2 constant    ", 0.0 , 0.0 , 0.0,0.0,ierflg);
   gMinuit->mnparm(13,"width gauss 2 E deps      ", 0.040,0.02,0.0,0.6,ierflg);
   gMinuit->mnparm(14,"width gauss 2 sqrt(E) deps", 0.052,0.01,-0.9,4.9,ierflg);
