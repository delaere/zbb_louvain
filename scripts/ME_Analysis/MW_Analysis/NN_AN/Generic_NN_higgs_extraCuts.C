...
#include "include.h"
#include "Read_input_extraCuts.h"

void Neural_net_E(const char *dy,const char *tt,const char *zz,const char *zh,TString name,int tag, int cut,TString NNStruct,int iterations, double massH, int jetMulti)
{
	...
	if(tag==1){Input(dy,N1,var1,sim,simu,1,0,0,cut,massH,jetMulti);}
	nn_vars *var2 = new nn_vars(N2);
	if(tag==2){Input(tt,N2,var2,sim,simu,1,0,0,cut,massH,jetMulti);}
	nn_vars *var3 = new nn_vars(N3);
	if(tag==3){Input(zz,N3,var3,sim,simu,1,0,0,cut,massH,jetMulti);}
	nn_vars *var4 = new nn_vars(N4);
	Input(zh,N4,var4,sim,simu,1,1,0,cut,massH,jetMulti);
	...
	evt_DY[i]=false;
	  ...
	  if(cut==0 && var1-> Mll[i]>76. && var1-> Mll[i]<106. && var1->tagj1[i]>0.244 && var1->tagj2[i]>0.244 && var1-> Mbb[i]>80. && var1-> Mbb[i]<150.){
	    if (jetMulti==0 && var1-> multi[i]==0){evt_DY[i]=true;}
	    if (jetMulti==1 && var1-> multi[i]==1){evt_DY[i]=true;}
	    }
	...
	evt_TT[i]=false;
	  ...
	  if(cut==0 && var2-> Mll[i]>76. && var2-> Mll[i]<106. && var2->tagj1[i]>0.244 && var2->tagj2[i]>0.244 && var2-> Mbb[i]>80. && var2-> Mbb[i]<150.){
	    if (jetMulti==0 && var2-> multi[i]==0){evt_DY[i]=true;}
	    if (jetMulti==1 && var2-> multi[i]==1){evt_DY[i]=true;}
	    }	  
	...
	evt_ZZ[i]=false;
	  ...
	  if(cut==0 && var3-> Mll[i]>76. && var3-> Mll[i]<106. && var3->tagj1[i]>0.244 && var3->tagj2[i]>0.244 && var3-> Mbb[i]>80. && var3-> Mbb[i]<150.){
	    if (jetMulti==0 && var3-> multi[i]==0){evt_DY[i]=true;}
	    if (jetMulti==1 && var3-> multi[i]==1){evt_DY[i]=true;}
	    }	  
	...
	evt_ZH[i]=false;
	  ...
	  if(cut==0 && var4-> Mll[i]>76. && var4-> Mll[i]<106. && var4->tagj1[i]>0.244 && var4->tagj2[i]>0.244 && var4-> Mbb[i]>80. && var4-> Mbb[i]<150.){
	    if (jetMulti==0 && var3-> multi[i]==0){evt_DY[i]=true;}
	    if (jetMulti==1 && var3-> multi[i]==1){evt_DY[i]=true;}
	    }	  
	  if(evt_ZH[i]==true){
