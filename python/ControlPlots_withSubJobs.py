...
parser.add_option("-n", "--whichSlice", dest="slice",
                  help="Defines slice of input files", metavar="SLICE")
...
def runTest(path, levels, outputname="controlPlots.root", ZjetFilter=False, checkTrigger=False, btagAlgo="SSV", onlyMu=False, onlyEle=False):
  ...
  files=[]
  beginSlice = int(options.slice)
  endSlice   = int(options.slice)+1
  print "begin slice = ", beginSlice
  print "end   slice = ", endSlice
  for fname in dirList[beginSlice:endSlice]:
...
def main(options):
  ...
  runTest(path=options.path,
          outputname=options.outputname,
          levels=levels,
          ZjetFilter=options.ZjetFilter,
          checkTrigger=options.checkTrigger,
          btagAlgo=options.btagAlgo,
          onlyMu=options.onlyMu,
          onlyEle=options.onlyEle)
